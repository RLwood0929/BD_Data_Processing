...
class dataChangeFuncion:
    ...
    轉換過程中，共用的函數與轉換規則對應函數
        ...
        class 內部函數呼叫使用
            ...
            search_pid_in_master_file   在MasterFile中搜尋產品ID，回傳需要的 value 值；dealer_id -> str；product_id -> str；data_date -> time format
            get_dp_type_in_kalist       從 KAList 工作表中搜尋 price type；dealer_id -> str；buyer_id -> str；data_date -> time format
        ...
        轉換過程中共用函數
            ...
            checkProdictIdInMasterFile  比對檔案中的 product id 存在於 master file 中；input_file_data -> str；dealer_id -> str

        轉換規則對應的函數
            函數名稱                     說明
            moveRule                    將原先欄位的值移動到搬移到新的欄位；input_data -> DataFrame；
            fixedValue                  欄位值固定為某些值；value -> str；row -> int
            changeTimeFormat            轉換欄位內容的時間格式；input_data -> DataFrame；input_col -> str；data_format -> str
            moveOrSearchUom             搬移或是轉換 Uom 值；input_data -> DataFrame；source_col -> str；dealer_id -> str；target_col -> str
            searchDP                    搜尋 Dp 資料，若在 kalist 中，則需篩選對應的 value；input_data -> DataFrame；dealer_id -> str
            searchCost                  搜尋 Cost 資料；input_data -> DataFrame；dealer_id -> str
            mergeColumns                多欄位值合併；input_data -> DataFrame；source_col -> str；value -> str
            lastTransactionDate         將 transaction date 依照升序排序後，取出最後一個值；input_data -> DataFrame；row -> int
    """
    ...
    def searchDP(self, input_data, dealer_id):
        ...
        for row_index, single_row in input_data.iterrows():
            ...
            col_price_type = {"DP":"DP(EA)", "KA":"KADP(EA)", "IVY":"IVY (EA)"}.get(price_type, "DP(EA)")
...
class SaleDataChange(dataChangeFuncion):
    ...
    def __init__ (self):
        ...
        self.change_result = {"Status" : self.change_status, "OutputFileName": None, "ErrorNum": 0,\
                         "Num" : 0, "ErrorReportFileName" : None, "ErrorReportPath" : None}
    ...
    def convert_data_in_rule(self):
        ...
        for column_index in self.change_rule:
            ...
            # 強制轉換規則
            if (target_col == "Gross Revenue") and (self.dealer_OUP_type == False):
                rule = "SearchDP"

            if (target_col == "Demand Class Desc") and (self.dealer_OUP_type == False):
                rule = "SearchCost"
    ...
    def convert_error_output_info(self):
        ...
        if error_report_flag:
            error_report_file_name = Config.SaleErrorReportFileName.replace\
                ("{DealerID}", str(self.dealer_id)).replace("{StartDate}", start_date).replace("{EndDate}", end_date)

            # 測試用
            error_report_folder_path = os.path.join("./datas")
            # error_report_folder_path = os.path.join(Config.ErrorReportPath, self.dealer_id, datetime.strftime(Config.SystemTime, "%Y%m"))
            ...
            try:
                ...
                # error_report_data.loc[:, self.transaction_date] = error_report_data[self.transaction_date].apply\
                #     (lambda x: x.strftime("%Y/%m/%d") if not pd.isnull(x) else x)
                # # 將 creation_date 欄位內容調整為：年/月/日
                # error_report_data.loc[:,self.creation_date] = error_report_data[self.creation_date].apply\
                #     (lambda x: x.strftime("%Y/%m/%d") if not pd.isnull(x) else x)
    ...
    def changeSaleFile(self, dealer_id, file_name):
        ...
        if dealer_index is not None:
            ...
            self.dealer_OUP_type = Config.DealerConfig[f"Dealer{dealer_index}"]["SaleFile"]["OUPType"]
...
class InventoryDataChange(dataChangeFuncion):
    ...
    def convert_data_in_rule(self):
        ...
        for column_index in self.change_rule:
            ...
            # Date Period 欄位為 Transaction Date 最後一天
                ...
                self.convert_output[target_col] = self.lastTransactionDate(input_data, len(input_data))

            else:
                self.sale_change_status = "NO"
                msg = f"{rule} 此轉換規則不再範圍中。"
                WSysLog("3", "ConvertDataInRule", msg)
                raise ValueError(msg)
    ...
    def convert_error_output_info(self):
        ...
        if error_report_flag:
            error_report_file_name = Config.InventoryErrorReportFileName.replace("{DealerID}", str(self.dealer_id))\
                .replace("{StartDate}", start_date)\
                .replace("{EndDate}", end_date)
...
if __name__ == "__main__":
    # aa = "AD279"
    # print(Config.DealerList)
    # dealer_index = Config.DealerList.index(aa)\
    #         if aa in Config.DealerList else None
    # print(dealer_index)
    # inventory_change = InventoryDataChange()
    # change_result = inventory_change.changeInventoryFile("1002317244", "1002317244_I_202410232000.xlsx")
    # sale_change = SaleDataChange()
